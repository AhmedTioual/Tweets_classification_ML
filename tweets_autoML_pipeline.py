...
@dag(
    dag_id='tweets_AutoML_pipeline',
    default_args=default_args,
    description='This DAG is for auto ML models',
...
def tweets_preprocessing_pipeline():
    ...
    def best_model(modelsDict):
        ...
        for model_file, mean_cv_score in modelsDict.items():
    ...
    # Function to save model and metrics to MongoDB
    def push_model_to_mongodb(data):
        
        db = "TweetsDataBase"
        collection = "TweetsClassificationModels"
        client = "mongodb+srv://mlteam:mlteam@cluster0.6y3bpz0.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
        ...
        try:
            ...
            # Inserting the data
            info = mycol.insert_one(data)
            print(f"Model saved with id: {info.inserted_id}")
            return True
        except Exception as e:
            print(f"An error occurred: {e}")
            return False
    
    # Define a mapping from model name (string) to the actual sklearn model class
    model_mapping = {
    ...
    @task
    def train_evaluate_model(X_train_path=None, y_train_path=None, X_val_path=None, y_val_path=None, model_name=None, model_save_dir='Models'):
            try:
                ...
                # Convert model name to actual model class
                ...
                return push_model_to_mongodb(data=data)
    ...
    train_evaluate_model_task = train_evaluate_model(
        X_train_path=feature_extraction_split_task['X_train_file'],
        y_train_path=feature_extraction_split_task['y_train_file'],
        X_val_path=feature_extraction_split_task['X_val_file'],
        y_val_path=feature_extraction_split_task['y_val_file'],
        model_name=best_model_task['best_model_file']
    )
    push_model_to_mongodb_task = push_model_to_mongodb(
        data=train_evaluate_model_task
        )
